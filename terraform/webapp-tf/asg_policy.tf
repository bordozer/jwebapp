resource "aws_autoscaling_policy" "scale_out_policy" {
  name = "${local.aws_service_name}-asg-scale-out-policy"
...
resource "aws_autoscaling_policy" "scale_in_policy" {
  name = "${local.aws_service_name}-asg-scale-in-policy"
  policy_type = "SimpleScaling"
  scaling_adjustment = -1
  adjustment_type = "ChangeInCapacity"
  cooldown = 180
...
resource "aws_autoscaling_policy" "cpu_target_tracking_policy" {
  name = "${local.aws_service_name}-asg-increase_instances_count-policy"
